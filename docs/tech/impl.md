...
src/
  ...
  typewriter.ts the Typewriter Mode extension and its switch
...
- **Move List Item** — accept exactly one collapsed caret; parse the contiguous list run into item subtrees, move the current subtree with adjacent siblings or across one parent boundary, then renumber the run and place the caret at its corresponding line and column. Blank lines, fences, headings, selections and multiple carets are hard boundaries or no-ops.
...
- **Table navigation** — the document-level capture listener handles Enter, Tab and Shift+Tab before the editor's default behavior. Pure table plans format through the existing renderer, skip the delimiter row, and add a column only when Tab moves past the right edge.
...
- **Typewriter Mode** — in memory, not in settings. Entering it switches the view to Source first.
