export default {
  // Commands
  Paste: "貼上",
  Copy: "複製",
  Cut: "剪下",
  "Paste as Plain Text": "貼上為純文字",
  "Font Family": "字型",
  "Font Size": "字型大小",
  Bold: "粗體",
  Italic: "斜體",
  Underline: "底線",
  Strikethrough: "刪除線",
  Highlight: "醒目提示",
  Superscript: "上標",
  Subscript: "下標",
  "Inline Code": "行內程式碼",
  "Inline Math": "行內公式",
  "Font Color": "字型色彩",
  "Highlight Color": "醒目提示色彩",
  "Clear Formatting": "清除所有格式",
  "Change Case": "變更大小寫",
  "Heading 1": "標題 1",
  "Heading 2": "標題 2",
  "Heading 3": "標題 3",
  "Heading 4": "標題 4",
  "Heading 5": "標題 5",
  "Heading 6": "標題 6",
  "Remove Heading": "轉為本文",
  "Bullet List": "項目符號",
  "Numbered List": "編號",
  "Number Headings": "編號標題",
  "Number Headings 1. 1.1. 1.1.1.": "編號標題 1. 1.1. 1.1.1.",
  "Number Headings 1) a) i)": "編號標題 1) a) i)",
  "Number Headings I. A. 1.": "編號標題 I. A. 1.",
  "No Numbering": "無編號",
  "Task List": "工作清單",
  Quote: "引文",
  "Increase Indent": "增加縮排",
  "Decrease Indent": "減少縮排",
  "Renumber List": "重新編號",
  "Sort Lines": "段落排序",
  "Move List Item Up": "上移清單項目",
  "Move List Item Down": "下移清單項目",
  "Swap Line Up": "上移一列",
  "Swap Line Down": "下移一列",
  "Align Left": "靠左對齊",
  "Align Center": "置中",
  "Align Right": "靠右對齊",
  "Align Justify": "左右對齊",
  "Horizontal Rule": "分隔線",
  Undo: "復原",
  Redo: "重複",
  "Find and Replace": "尋找及取代",
  "Internal Link": "內部連結",
  "External Link": "外部連結",
  Embed: "內嵌",
  Tag: "標籤",
  "Block Reference": "區塊參照",
  Callout: "標註",
  "Note Callout": "註解標註",
  "Abstract Callout": "摘要標註",
  "Info Callout": "資訊標註",
  "Tip Callout": "提示標註",
  "Success Callout": "成功標註",
  "Question Callout": "問題標註",
  "Warning Callout": "警告標註",
  "Failure Callout": "失敗標註",
  "Danger Callout": "危險標註",
  "Bug Callout": "缺陷標註",
  "Example Callout": "範例標註",
  "Quote Callout": "引文標註",
  "Code Block": "程式碼區塊",
  "Math Block": "公式區塊",
  Table: "表格",
  "Convert Text to Table": "文字轉換為表格",
  Comment: "註解",
  "Image Size": "圖片大小",
  "100 px": "100 像素",
  "200 px": "200 像素",
  "300 px": "300 像素",
  "400 px": "400 像素",
  "600 px": "600 像素",
  "Original Size": "原始大小",
  Caption: "題註",
  "All Images": "所有圖片",
  "All Images 100 px": "所有圖片 100 像素",
  "All Images 200 px": "所有圖片 200 像素",
  "All Images 300 px": "所有圖片 300 像素",
  "All Images 400 px": "所有圖片 400 像素",
  "All Images 600 px": "所有圖片 600 像素",
  "All Images Original Size": "所有圖片 原始大小",
  "Alt Text": "替代文字",
  "Reset Picture": "重設圖片",
  "Convert Syntax": "轉換語法",
  Footnote: "腳注",
  "Attach File": "附加檔案",
  "Emoji & Symbols": "表情符號與符號",
  "Date and Time": "日期與時間",
  "Table of Contents": "目錄",
  "Show Whitespace": "顯示空格",
  "Live Preview/Source": "即時預覽/原始碼",
  "Show Line Numbers": "顯示行號",
  "Focus Mode": "專注模式",
  "Zen Mode": "禪模式",
  "Typewriter Mode": "打字機模式",
  "Readable Line Length": "可讀行寬",
  "Navigation Pane": "導覽窗格",
  "Zoom In": "放大",
  "Zoom Out": "縮小",
  "100%": "100%",
  "Split Right": "向右分割",
  "Split Down": "向下分割",
  Collapse: "摺疊",
  Expand: "展開",
  "Collapse All": "全部摺疊",
  "Expand All": "全部展開",
  "Merge Lines": "合併列",
  "Split Lines": "分割列",
  "Reverse Lines": "反轉行",
  Duplicate: "重複",
  "Sort List": "排序清單",
  "Sort Headings": "排序標題",
  "Delete Rows": "刪除列",
  "Delete Columns": "刪除欄",
  "Delete Table": "刪除表格",
  "Insert Rows Above": "在上方插入列",
  "Insert Rows Below": "在下方插入列",
  "Insert Columns to the Left": "在左側插入欄",
  "Insert Columns to the Right": "在右側插入欄",
  "Move Row Up": "列上移",
  "Move Row Down": "列下移",
  "Move Column Left": "欄左移",
  "Move Column Right": "欄右移",
  "Format Tables": "格式化表格",
  "Format Table": "格式化此表格",
  "Format All Tables": "格式化所有表格",
  "Align Column Left": "欄靠左對齊",
  "Align Column Center": "欄置中",
  "Align Column Right": "欄靠右對齊",
  "Sort Rows": "列排序",
  "Sort Rows A to Z": "由 A 到 Z 排序",
  "Sort Rows Z to A": "由 Z 到 A 排序",
  "Remove Duplicate Rows": "刪除重複列",
  "Transpose Table": "轉置表格",
  "Convert Table to Text": "將表格轉換為文字",
  "Paste as Table": "貼上為表格",
  "Copy as": "複製為",
  "Copy as TSV": "複製為 TSV",
  "Copy as CSV": "複製為 CSV",
  "Copy as JSON": "複製為 JSON",
  "Copy as Markdown": "複製為 Markdown",
  "Paste URI as Link": "貼上 URI 為連結",
  "Smart Punctuation": "智慧標點",
  "CJK Spacing": "中日韓間距",
  "Clean Up": "清理",
  "Remove Trailing Spaces": "移除行尾空白",
  "Collapse Blank Lines": "合併連續空白行",
  "Convert Bare URLs": "轉換裸 URL",
  "Normalize Emphasis & Strong": "統一強調與粗體",
  "Normalize Bullet Style": "統一項目符號樣式",

  // Tabs and groups
  Home: "常用",
  Insert: "插入",
  View: "檢視",
  Utilities: "公用工具",
  Pinned: "已釘選",
  Clipboard: "剪貼簿",
  Font: "字型",
  Paragraph: "段落",
  Styles: "樣式",
  Editing: "編輯",
  Links: "連結",
  Blocks: "區塊",
  Picture: "圖片",
  "Media & Symbols": "媒體與符號",
  Reference: "參照",
  Show: "顯示",
  Views: "檢視",
  Immersive: "沉浸式",
  Zoom: "縮放",
  Window: "視窗",
  Outlining: "大綱",
  "Rows & Columns": "列與欄",
  Format: "格式",
  Alignment: "對齊方式",
  Data: "資料",
  Lines: "行",
  Sort: "排序",
  Normalize: "正規化",

  // Positions
  Fixed: "底部",
  Following: "跟隨",
  Top: "頂端",

  // Settings
  Toolbar: "工具列",
  General: "一般",
  Desktop: "桌面",
  Mobile: "行動裝置",
  "Compact bar pinned to the bottom of the editor.":
    "精簡工具列固定在編輯器底部。",
  "Compact bar above the selection.": "精簡工具列顯示在所選範圍上方。",
  "Ribbon pinned above the editor.": "功能區固定在編輯器上方。",
  "Enable on Mobile": "在行動裝置上啟用",
  "Hides every bar on mobile without clearing its positions.":
    "在行動裝置上隱藏所有工具列，但不會清除各位置的開關狀態。",
  "Keyboard navigation": "鍵盤導航",
  "Enter moves down a cell, Tab to the next one and Shift+Tab back, and Shift+Enter starts a new line inside the cell. Tab at the right edge adds a column and Enter on the last row adds a row below it. Source mode only.":
    "Enter 移到下方儲存格，Tab 移到下一個，Shift+Tab 回到上一個，Shift+Enter 在儲存格內換行；在最右欄按 Tab 會新增一欄，在最後一列按 Enter 會在其下方新增一列。僅適用於原始碼模式。",
  "Pad cells with spaces": "以空格填滿儲存格",
  "Lines the pipes of each column up by padding cells with spaces. Live Preview always pads, so turning this off makes tables flip between the two styles as you edit.":
    "以空格填滿儲存格，讓各欄的直線對齊。即時預覽一律會填滿，關閉此項會讓表格在編輯時於兩種樣式之間來回切換。",
  "Sort on header click": "按一下標題排序",
  "Click a table header in Reading view to sort its rows. The file is not modified.":
    "在閱讀模式中按一下表格標題即可排序其列，不會修改檔案。",
  "Pin a command": "釘選命令",
  "No pinned commands yet.": "尚未釘選任何命令。",
  "Change icon": "變更圖示",
  "Manage Pinned": "管理已釘選命令",
  "Manage pinned commands and groups.": "管理已釘選的命令和分組。",
  Done: "完成",
  Cancel: "取消",
  "Rename group": "重新命名分組",
  "Move to": "移至",
  "New group…": "新增分組…",
  Delete: "刪除",

  // Toolbar, panels and pickers
  "More commands": "更多命令",
  "Hidden by width": "因寬度不足而隱藏",
  "Search…": "搜尋…",
  "Search characters": "搜尋字元",
  "No matches": "無相符項目",
  "No Color": "無色彩",
  "More colors…": "其他色彩…",
  UPPERCASE: "全部大寫",
  lowercase: "全部小寫",
  "Capitalize Each Word": "每個字首大寫",
  "tOGGLE cASE": "切換大小寫",
  Default: "預設",
  Serif: "襯線",
  "Sans Serif": "無襯線",
  Monospace: "等寬",
  Cursive: "手寫",
  "Search commands to pin": "搜尋要釘選的命令",
  "Icon for {name}": "{name}的圖示",
  "{tab} · {group}": "{tab} · {group}",

  // Notices
  "{name} failed. See the developer console for details.":
    "{name} 執行失敗。詳細資料請參閱開發人員主控台。",
  "Clipboard is not a table. Copy two or more rows of tab- or comma-separated values first.":
    "剪貼簿內容不是表格。請先複製兩列或以上以 Tab 或逗號分隔的資料。",
  "This note has no headings to list.": "此筆記沒有可列出的標題。",
  "Select two or more rows of tab- or comma-separated values first.":
    "請先選取兩列或以上以 Tab 或逗號分隔的資料。",
  "Clipboard does not contain a valid URI.": "剪貼簿中沒有有效的 URI。",
  "No duplicate rows found.": "找不到重複列。",
  "Removed duplicate rows: {count}": "已刪除重複列：{count}",
  "Put the cursor inside a table first.": "請先將游標放在表格內。",
  "Table copied as {format}.": "已複製表格為 {format}。",
} satisfies Record<string, string>;
