...
**138 built-in commands** across 5 tabs and 25 groups, plus the Pinned tab. Drop-down items stay under their parent buttons, while the other commands are registered in the command palette, so users can assign their own shortcuts. That leaves **126** palette entries: the 12 left out are the 11 drop-down parents and the emoji panel, which only open a menu instead of acting.
...
- Reordering, hiding or showing the 138 built-in commands.
...
| **Command** | An action the toolbar runs. **138 built-in commands** plus any **pinned command**. |
